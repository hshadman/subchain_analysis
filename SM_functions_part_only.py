...
def compute_30mer_data_from_seq_name(seq_name,k_frac, h5_file_path):
    ...
    #REMINDER: edits made, such that the h5 files are NO LONGER saved. below codes on h5_file_path is just not used. 
    ...
    for start_res in range(1, n_residues - k + 2):  # Ensures we don't go out of bounds
        ...
        subchain_std_Rs.append(np.std(running_inst_ratio))
    ...
    subchain_std_Rs_for_SM = [combined_subchain_metrics.subchain_std_Rs.mean()]
    ...
    subchain_std_RSA_for_SM = [combined_subchain_metrics.subchain_std_RSA.mean()]
