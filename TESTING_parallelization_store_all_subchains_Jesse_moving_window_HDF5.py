...
def process_sequence(seq_n):
    hdf5_path = f'./subchain_repository_hdf5/{seq_n}.h5'
    df = compute_30mer_data_from_seq_name(seq_n, 3, hdf5_path)
    return df
...
if __name__ == "__main__":
    try:
        ...
        # Divide the seq_name list into 5 chunks containing ~5600 sequences for parallelization
        seq_name_chunks = np.array_split(idrome_prop_flucs.seq_name.values, 100)
        
        # Process the first chunk in parallel
        results = pool.map(process_sequence, seq_name_chunks[0])
        
        # Concatenate all DataFrames returned from processes
        master_df = pd.concat(results)
        master_df.to_csv('./TESTING_parallelization_store_all_subchains_Jesse_moving_window_HDF5.csv')
        print('Done with chunk')
    finally:
        pool.close()
        pool.join()
#divide the seq_name list to 5 chunks containing ~5600 sequences for PARALLELIZATION
